...
FIXES applied:
  1. await tf.ready() before any tensor ops
  2. proper null/empty-array guard on feeds ([] is truthy — was bypassing re-fetch)
  3. status element updated on every state transition, including errors
  4. startPredictionEngine exported AND called safely via window.onload fallback
  5. removed duplicate DOMContentLoaded double-init (handled in dashboard.html inline script)
  6. fixed field mapping comment inconsistency (1 = occupied, matching dashboard.js)
  7. added retry logic when initial fetch returns no data
*/
...
const FETCH_INTERVAL    = 30_000;   // 30 s
const TRAIN_INTERVAL    = 60_000;   // 60 s
const PREDICT_INTERVAL  = 1_000;    //  1 s
const MIN_FEEDS_NEEDED  = 5;        // reduced from 20 so sparse channels still work
const FETCH_RETRY_DELAY = 8_000;    // retry after 8 s if first fetch is empty

const HW_THREADS   = (typeof navigator !== 'undefined' && navigator.hardwareConcurrency) ? navigator.hardwareConcurrency : 4;
const SMALL_DEVICE = HW_THREADS <= 2;
const TRAIN_TIME_CAP_MS = SMALL_DEVICE ? 120 : 180;
...
let model          = null;
let normalizer     = null;
let lastFetchedFeeds = null;   // null = never fetched; [] = fetched but empty
...
// ============================================================================
// Status helper — always update UI so user can see what's happening
// ============================================================================
function setStatus(msg, isError = false) {
  console.log('[PredictionEngine]', msg);
...
// ============================================================================
async function fetchThingSpeakData() {
  setStatus('Fetching sensor data…');
  try {
    const res = await fetch(PRED_THINGSPEAK_URL);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const json = await res.json();
    if (!json.feeds || json.feeds.length === 0) {
      setStatus('No feeds returned — will retry', true);
      return [];
    }
    const feeds = json.feeds.slice(-100);
    lastFetchedFeeds = feeds;
    setStatus(`Fetched ${feeds.length} records`);
    return feeds;
  } catch (err) {
...
// ============================================================================
// prepareFeatures
// Field mapping (matching Arduino + dashboard.js):
//   field1 = gasValue (analog 0–1023)
//   field2 = distance cm (ultrasonic)
//   field3 = IR1  1 = OCCUPIED, 0 = VACANT
//   field4 = IR2  1 = OCCUPIED, 0 = VACANT
//   field5 = IR3  1 = OCCUPIED, 0 = VACANT
// Features: [ir1, ir2, ir3, gas, ultrasonic, hour, minute, timeIndex]
// Label   : occupancy fraction 0..1
// ============================================================================
function prepareFeatures(feeds) {
  if (!feeds || feeds.length === 0) return null;
  const rows = [];
  for (let i = 0; i < feeds.length; i++) {
    const f = feeds[i];
    const gas       = parseFloat(f.field1) || 0;
    const ultrasonic= parseFloat(f.field2) || 0;
    const ir1       = parseInt(f.field3)   || 0;   // 1 = occupied
    const ir2       = parseInt(f.field4)   || 0;
    const ir3       = parseInt(f.field5)   || 0;
    const ts        = new Date(f.created_at);
    const hour      = ts.getHours();
    ...
    rows.push({
      x:   [ir1, ir2, ir3, gas, ultrasonic, hour, minute, i],
      y:   occupancy,
      raw: { s1: ir1, s2: ir2, s3: ir3, gas, ultrasonic, hour, minute, timeIndex: i, created_at: f.created_at }
    });
  }
  return rows;
}
...
function computeNormalizer(rows) {
  ...
  rows.forEach(r => r.x.forEach((v, i) => {
...
function normalizeRow(x, norm) {
  return x.map((v, i) => (v - norm.min[i]) / (norm.max[i] - norm.min[i]));
}
...
function createModel(inputShape) {
  ...
  m.add(tf.layers.dense({ units: 1,      activation: 'sigmoid' }));
  m.compile({ optimizer: tf.train.adam(0.01), loss: 'meanSquaredError', metrics: ['mse'] });
  console.log(`[PredictionEngine] model created: ${units1}→${units2}→1`);
  return m;
}

// ============================================================================
// Training  (FIX: await tf.ready() before tensor ops)
// ============================================================================
async function trainModel(mdl, X, y) {
  // FIX 1: ensure TF backend is initialized before creating tensors
  await tf.ready();
  ...
  const timeCap = {
    onBatchEnd: async () => {
      if (performance.now() - start > TRAIN_TIME_CAP_MS) {
        mdl.stopTraining = true;
        stopped = true;
      }
    }
  };
  ...
  try {
    await mdl.fit(xs, ys, { epochs: maxEpochs, batchSize, shuffle: true, callbacks: [timeCap] });
  } catch (err) {
    console.warn('[PredictionEngine] Training interrupted', err);
  } finally {
    xs.dispose();
    ys.dispose();
  }
  ...
  const elapsed = Math.round(performance.now() - start);
  return { stoppedEarly: stopped, elapsedMs: elapsed };
}
...
function estimateConfidence(mdl, X, y) {
  try {
    const preds = tf.tidy(() => {
      const xs = tf.tensor2d(X);
      const p  = mdl.predict(xs);
      return Array.from(p.dataSync());
    });
    let mse = 0;
    preds.forEach((p, i) => { const d = p - y[i]; mse += d * d; });
    mse /= preds.length;
    const rmse = Math.sqrt(mse);
    const conf = Math.round(Math.min(100, Math.max(0, (1 - rmse) * 100)));
    return conf;
  } catch { return 0; }
}

// ============================================================================
// Trend array (next N minutes)
// ============================================================================
function createTrendArray(numPoints = 10) {
  if (!lastPrepared || !normalizer) return [];
  const lastRaw = lastPrepared.raw[lastPrepared.raw.length - 1];
  const lastOcc = lastPrepared.y[lastPrepared.y.length - 1];

  return Array.from({ length: numPoints }, (_, i) => {
    const t = new Date(lastRaw.created_at);
    t.setMinutes(t.getMinutes() + i + 1);
    const sProxy = lastOcc > 0.5 ? 1 : 0;
    const rawX  = [sProxy, sProxy, sProxy, lastRaw.gas, lastRaw.ultrasonic, t.getHours(), t.getMinutes(), lastRaw.timeIndex + i + 1];
    const normX = normalizeRow(rawX, normalizer);
    const p     = predictOccupancy(normX);
    return isFinite(p) ? Math.max(0, Math.min(1, parseFloat(p.toFixed(3)))) : 0;
  });
}
...
function animateNumberEl(el, target, duration = 600) {
  if (!el) return;
  const start     = parseFloat(el.dataset.animTarget) || 0;
  const end       = target * 100;
  el.dataset.animTarget = end;
  const t0 = performance.now();
  (function step(now) {
    const t = Math.min(1, (now - t0) / duration);
    el.textContent = Math.round(start + (end - start) * t) + '%';
    if (t < 1) requestAnimationFrame(step);
  })(t0);
}
...
function setCardColor(el, value) {
  if (!el) return;
  el.classList.remove('green', 'yellow', 'red');
  el.classList.add(value < 0.4 ? 'green' : value < 0.7 ? 'yellow' : 'red');
}
...
// ============================================================================
// Build + train orchestrator
// ============================================================================
async function buildAndTrainIfNeeded() {
  // FIX 2: feeds must be a non-empty array — [] is truthy so we must check length
  let feeds = (lastFetchedFeeds && lastFetchedFeeds.length > 0)
    ? lastFetchedFeeds
    : await fetchThingSpeakData();

  if (!feeds || feeds.length < MIN_FEEDS_NEEDED) {
    setStatus(`Not enough data (${feeds ? feeds.length : 0}/${MIN_FEEDS_NEEDED} feeds)`, true);
    return;
  }

  const rows = prepareFeatures(feeds);
  if (!rows || rows.length === 0) { setStatus('Feature prep failed', true); return; }

  normalizer  = computeNormalizer(rows);
  const X     = rows.map(r => normalizeRow(r.x, normalizer));
  const y     = rows.map(r => r.y);
  const raw   = rows.map(r => r.raw);
  lastPrepared = { X, y, raw };

  if (!model) model = createModel(X[0].length);

  // train async without blocking UI
  (async () => {
    try {
      const info = await trainModel(model, X, y);
      const conf = estimateConfidence(model, X, y);
      ...
      setStatus(`Trained in ${info.elapsedMs}ms · confidence ${conf}%`);
    } catch (err) {
      setStatus(`Training failed: ${err.message}`, true);
      console.error('[PredictionEngine] training error', err);
    }
  })();
}

// ============================================================================
// Prediction loop  (1 Hz)
// ============================================================================
function runPredictionLoop() {
  setInterval(() => {
    ...
    const trend  = createTrendArray(10);
    const lastVal = trend.length ? trend[trend.length - 1] : 0;

    const elVal  = document.getElementById(UI.value);
    const elCard = document.getElementById(UI.card);
    if (elVal)  animateNumberEl(elVal, lastVal, 500);
    if (elCard) setCardColor(elCard, lastVal);

    const labels = trend.map((_, i) => `+${i + 1}m`);
    createOrUpdateTrendChart(labels, trend.map(v => Math.round(v * 100) / 100));
...
// ============================================================================
// Engine entry point  (FIX 3: await tf.ready() before anything else)
// ============================================================================
async function startPredictionEngine() {
  if (engineStarted) return;   // guard against double-start
  engineStarted = true;

  setStatus('Initializing TF.js…');
  try {
    await tf.ready();           // FIX: wait for WebGL / CPU backend
    setStatus('TF.js ready');
  } catch (err) {
    setStatus('TF.js init failed', true);
    console.error('[PredictionEngine] tf.ready() error', err);
    return;
  }
  ...
  // retry once if channel returned nothing
  if (!lastFetchedFeeds || lastFetchedFeeds.length === 0) {
    setStatus(`No data — retrying in ${FETCH_RETRY_DELAY / 1000}s…`, true);
  ...
  setInterval(buildAndTrainIfNeeded,   TRAIN_INTERVAL);
...
// ============================================================================
// Auto-init (FIX 4: single clean init path — checks tf existence + DOM state)
// dashboard.html's inline DOMContentLoaded listener is unreliable when scripts
// are at bottom of <body> because the event has already fired. We use
// window.onload as a safe fallback, plus an immediate call if already loaded.
// ============================================================================
function _tryAutoStart() {
  if (typeof tf === 'undefined') {
    // tf not yet defined — wait and retry
